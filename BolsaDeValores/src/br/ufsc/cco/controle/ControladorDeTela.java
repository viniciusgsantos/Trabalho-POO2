...
public class ControladorDeTela {
	...
	public void setCelula(String linha, Atributo coluna, String dado,
			Color color) {
		switch (linha) {
	...
	public void updateScreen() {
		for (Entry<String, Ativo> entry : ControladorDeDados.getInstance()
				.getAtivos().entrySet()) {
			Ativo ativo = entry.getValue();
			if (ativo.isUpdated()) {
				setCelula(ativo.getNome(), Atributo.CLOSE,
						String.format("%.2f", ativo.getClose()),
						ativo.getColor());
				setCelula(ativo.getNome(), Atributo.SMA,
						String.format("%.2f", ativo.getSma()), null);
				setCelula(ativo.getNome(), Atributo.SITUACAO,
						ativo.getSituacao(), null);
